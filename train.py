...
def train(args, model, train_loader, optimizer, epoch, logger):
    ...
    counter = 0
    for i, (image, label, img_file) in enumerate(train_loader):
...
def single_scale_test(model, test_loader, test_list, save_dir):
    ...
    for idx, (image, img_file) in enumerate(test_loader):
        image = image.cuda()
        _, _, H, W = image.shape
        results = model(image)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save-dir', help='output folder', default='train_results/RCF')
